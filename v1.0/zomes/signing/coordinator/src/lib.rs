...
///
/// Creates two links:
/// - FileHashToSignature: file_hash bytes -> ActionHash (for verification lookup)
/// - AgentToSignature: AgentPubKey -> ActionHash (for signing history)
#[hdk_extern]
pub fn create_signature(record: SignatureRecord) -> ExternResult<ActionHash> {
    ...
    // Link from file_hash for verification lookup.
    // Use an ExternalHash (AnyLinkableHash) from the raw file_hash bytes.
